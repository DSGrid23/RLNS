...
def test_one_user(x):
    ...
    # Get all items and filter out training items
    all_items = set(range(0, n_items))
    test_items = list(all_items - set(training_items))

    # Generate rank list and calculate performance
...
def test(model, user_dict, n_params, mode='test'):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        user_batch_rating_uid = [
            (rating, user, train_user_set, test_user_set, n_items)
            for rating, user in zip(rate_batch, user_list_batch)
        ]
